...
const orDefault = <T,>(value: T | undefined | null, fallback: T): T => value || fallback;
...
export const proxyFormFromExisting = (p: Proxy): ProxyConfig => ({
  ...
  local_address: orDefault(p.local_address, ""),
  local_port: orDefault(p.local_port, "10051"),
  address: orDefault(p.address, "127.0.0.1"),
  port: orDefault(p.port, "10051"),
  allowed_addresses: orDefault(p.allowed_addresses, ""),
  tls_connect: orDefault(p.tls_connect, 1),
  tls_accept: orDefault(p.tls_accept, 1),
  tls_issuer: orDefault(p.tls_issuer, ""),
  tls_subject: orDefault(p.tls_subject, ""),
  tls_psk_identity: orDefault(p.tls_psk_identity, ""),
  tls_psk: "",
  custom_timeouts: orDefault(p.custom_timeouts, 0),
  timeout_zabbix_agent: orDefault(p.timeout_zabbix_agent, ""),
  timeout_simple_check: orDefault(p.timeout_simple_check, ""),
  timeout_snmp_agent: orDefault(p.timeout_snmp_agent, ""),
  timeout_external_check: orDefault(p.timeout_external_check, ""),
  timeout_db_monitor: orDefault(p.timeout_db_monitor, ""),
  timeout_http_agent: orDefault(p.timeout_http_agent, ""),
  timeout_ssh_agent: orDefault(p.timeout_ssh_agent, ""),
  timeout_telnet_agent: orDefault(p.timeout_telnet_agent, ""),
  timeout_script: orDefault(p.timeout_script, ""),
  timeout_browser: orDefault(p.timeout_browser, ""),
});
...
type ProxySetter = <K extends keyof ProxyConfig>(key: K, value: ProxyConfig[K]) => void;

const ProxyTabGeneral = ({
  form,
  set,
  proxyGroups,
  isActive,
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              onChange={(_, c) =>
                set("tls_accept", c ? form.tls_accept | TLS_NONE : form.tls_accept & ~TLS_NONE)
              }
            />
          }
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              onChange={(_, c) =>
                set("tls_accept", c ? form.tls_accept | TLS_PSK : form.tls_accept & ~TLS_PSK)
              }
            />
          }
        ...
        <FormControlLabel
          control={
            <Switch
              ...
              onChange={(_, c) =>
                set("tls_accept", c ? form.tls_accept | TLS_CERT : form.tls_accept & ~TLS_CERT)
              }
            />
          }
...
const computeProxyEncryptionVisibility = (form: ProxyConfig) => {
  ...
  return {
    isActive,
    showPsk: (connectEncrypted && form.tls_connect === TLS_PSK) || acceptPsk,
    showCert: (connectEncrypted && form.tls_connect === TLS_CERT) || acceptCert,
  };
};

const canSubmitProxyForm = (form: ProxyConfig, isActive: boolean): boolean =>
  !!form.name.trim() &&
  (isActive || (!!form.address.trim() && !!form.port.trim())) &&
  (!form.proxy_groupid || !!form.local_address.trim());

const ProxyTabTimeouts = ({ form, set }: { form: ProxyConfig; set: ProxySetter }) => (
  <Stack spacing={2} sx={{ mt: 1 }}>
    <FormControlLabel
      control={
        <Switch
...
export const ProxyFormDialog = ({
  ...
  const { isActive, showPsk, showCert } = computeProxyEncryptionVisibility(form);
  const canSubmit = canSubmitProxyForm(form, isActive);
  ...
  return (
    <Dialog open={open} onClose={onCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        {tab === 0 && (
          <ProxyTabGeneral form={form} set={set} proxyGroups={proxyGroups} isActive={isActive} />
        )}
        {tab === 1 && (
          <ProxyTabEncryption
            form={form}
        ...
        {tab === 2 && <ProxyTabTimeouts form={form} set={set} />}
      </DialogContent>
      <DialogActions>
