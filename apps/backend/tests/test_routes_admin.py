...
def test_list_user_groups():
    ...
    with patch("api.routes.admin.zadmin_bot", mock), TestClient(app) as c:
        r = c.get("/user-groups")
...
def test_create_api_token_success():
    ...
    with patch("api.routes.admin.zadmin_bot", mock), TestClient(app) as c:
        r = c.post("/api-tokens", json={"name": "CI", "userid": "2", "expires_at": 0})
    assert r.status_code == 200
...
def test_test_portal_ldap_connection_ok():
    ...
    with (
        patch("api.routes.admin.zadmin_bot", mock),
        patch("api.routes.admin.get_portal_ldap_config", return_value={}),
        patch("api.routes.admin.test_portal_ldap_connection", return_value=(True, "ok")),
    ):
...
def test_test_portal_ldap_user_not_found():
    mock, app = _bot()
    from ldap_auth import LdapUserNotFoundError

    with (
        patch("api.routes.admin.zadmin_bot", mock),
        patch("api.routes.admin.get_portal_ldap_config", return_value={}),
        patch("api.routes.admin.test_portal_ldap_connection", return_value=(True, "ok")),
        patch("ldap_auth._do_ldap_auth", side_effect=LdapUserNotFoundError("joe")),
        TestClient(app) as c,
    ):
        r = c.post(
            "/admin/auth/portal-ldap/test",
