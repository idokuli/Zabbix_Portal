...
- **`@mui/material`'s `Autocomplete` (via `useAutocomplete.js`) has a real upstream null-ref bug** that surfaces as a full-page crash: `Cannot read properties of null (reading 'removeAttribute')`. Verified present in every published version from v5.18.0 through the current v9.3.1 (pulled and inspected each package directly) — v9.1.0+ has a guard before the crashing line, `if (inputRef.current == null && listboxRef.current == null) return;`, but it only bails when **both** refs are null; if just `inputRef.current` goes null (e.g. the `<input>` unmounts because its `Dialog` is closing) while `listboxRef.current` is still non-null (the Popper-portal listbox has separate unmount timing), it falls through to `inputRef.current.removeAttribute(...)` and throws. v5.18.0–v9.0.1 have **no guard at all** before that call — strictly worse. **There is no version to upgrade or downgrade to that avoids this** — don't try that fix again without first pulling the target version's `useAutocomplete.js` and checking the guard yourself (`npm pack @mui/material@<version>` and grep for `removeAttribute`).
  ...
  - Mitigation in place (not a real fix — the bug is still in MUI's code): every `multiple` `Autocomplete` in this codebase sets `disableCloseOnSelect` to cut down on listbox mount/unmount churn, and `HostGroupsTab.tsx`/`TemplateGroupsTab.tsx` each keep an `openRequestRef` counter, bumped on close (`closeDialog()`) and captured at the start of `openDialog()`, so a late-resolving fetch checks `requestId === openRequestRef.current` before touching `options`/`value` state. Keep both patterns when touching these dialogs, and apply the same pair to any new dialog that combines an async-loaded `multiple` `Autocomplete` with a `Dialog` the user can close mid-fetch.
