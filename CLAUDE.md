...
- **Every portal team has a same-named Zabbix host group, kept in sync automatically** — `ZabbixSync.push_team()` (`ZabbixSync/push.py`) creates it (or reuses an existing one) when a team is created, and `push_host_to_team()`/`remove_host_from_team()` add/remove a host from it whenever `POST`/`DELETE /teams/{team_id}/hosts` runs (alongside a parallel `team:<name>` Zabbix host tag used by `team_hostname_filter()`). This is a real, deliberately-maintained relationship — not a naming coincidence — and is the first half of what `team_group_names()` in `api/deps.py` returns (root/auditor get `None` → unrestricted, mirroring `team_hostname_filter()`'s convention).
  - **A team's group set isn't limited to just that one auto-created group.** The `team_host_groups (team_id, group_name)` table (`Database.py`) lets a Team Lead+ explicitly link any *pre-existing* Zabbix host group to a team — e.g. one created directly in Zabbix, or a shared functional group like "Linux servers" — via `POST`/`DELETE /teams/{team_id}/groups` (`User_Management.link_team_group()`/`unlink_team_group()`). Each `TeamCard` shows a **"Linked Host Groups"** chip list (read-only display, × per chip to unlink) fed by `linkedGroupsByTeam: Record<number, string[]>` — fetched centrally in `Teams/index.tsx`'s `load()` (one `api.getTeamGroups(t.id)` call per team, alongside the teams/hosts/users/groups fetch) rather than per-card, so a successful link/unlink just needs a `load(true)` to refresh every card at once. *Adding* a link uses the same interaction pattern as assigning a host to a team — an **"Assign Group"** button on the card opens `AssignGroupsToTeamDialog` (`Teams/index.tsx`), a searchable multi-select checklist (mirrors `AssignHostsDialog`'s "Individual Hosts" tab) sourced from the *unscoped* `api.listHostGroups()` call (`allHostGroups` state) minus the team's own name and its already-linked groups, calling `api.linkTeamGroup()` per selected row on submit. `team_group_names()` unions each team's own name with `um.list_team_linked_groups(team_id)`, in that order, deduped across teams — so a single-team user *can* see more than one group in "Filter by group" once groups are linked; the `display_order` caveat below now only describes the ordering *between different teams*, not a hard 1-group-per-team ceiling.
  - `GET /dc/host-groups?scope=mine` (used by the Problems/Hosts "Filter by group" pickers) filters+orders `DataCollection_Manager.list_host_groups()`'s result by `team_group_names()`'s output — **filtering happens in the route, not the manager**, same reasoning as the `team_hostname_filter()` note above: `list_host_groups()` has a single global 5-minute cache key shared by every caller, so team-filtering inside the manager would corrupt the cache for the three admin-only consumers (Data Collection, Maintenance, User Groups tabs) that call it *without* `scope=mine` and need every group. Never add team-filtering inside `DataCollection_Manager` itself for this reason.
  - `display_order` controls ordering between a user's different teams (only visible for a user in 2+ teams — the groups *within* one team, own name plus links, aren't independently orderable, they're grouped together right after the team's own name). It's set via `PUT /teams/{team_id}/display-order` (Team Lead+), driven by the Teams page's **Group Order** dialog (`apps/frontend/src/views/Teams/GroupOrderDialog.tsx`, opened from `TeamsHeader`) — a single up/down-arrow list, not a per-`TeamCard` field, so the whole reorderable set is visible at once. `GET /teams/mine` (`api/routes/teams.py`) backs it: root/auditor get every team (they have no personal team membership of their own to scope to — mirrors `is_global_viewer()`'s "unrestricted" convention elsewhere), everyone else gets only the teams they belong to via `um.get_user_teams_ordered()`. Each arrow click re-persists **every** row's `display_order` as its new array index (not just the two swapped rows) via `Promise.all` over `setTeamDisplayOrder()` calls — this normalizes away any stale/gapped/duplicate values from earlier edits rather than needing to special-case them.
